...
#[inline]
fn poseidon2_compress_8_to_4_circuit<P, const D: usize>(
    ...
    P: RichField + Extendable<D>,
{
    ...
    let mut state = [zero; WIDTH_8];

    for i in 0..OUT_8 {
        state[i] = builder.select(swap, right.elements[i], left.elements[i]);
        state[i + OUT_8] = builder.select(swap, left.elements[i], right.elements[i]);
    }
    ...
    for &rc in INTERNAL_CONSTANTS_8.iter() {
        state[0] = builder.add_const(state[0], P::from_canonical_u64(rc));
...
#[inline]
fn sbox_p_circuit<P, const D: usize>(builder: &mut CircuitBuilder<P, D>, x: Target) -> Target
where
    P: RichField + Extendable<D>,
{
...
{
    for item in state.iter_mut() {
        *item = sbox_p_circuit(builder, *item);
    }
}

#[inline]
fn add_rc_8_circuit<P, const D: usize>(
    builder: &mut CircuitBuilder<P, D>,
    state: &mut [Target; WIDTH_8],
    round: usize,
) where
...
#[inline]
fn internal_linear_layer_8_circuit<P, const D: usize>(
    builder: &mut CircuitBuilder<P, D>,
    state: &mut [Target; WIDTH_8],
) where
    P: RichField + Extendable<D>,
{
    let sum = builder.add_many(state.iter().copied());
    for i in 0..WIDTH_8 {
        state[i] =
            builder.mul_const_add(P::from_canonical_u64(MATRIX_DIAG_8_U64[i]), state[i], sum);
    }
}

#[inline]
fn external_linear_layer_hl_8_circuit<P, const D: usize>(
    builder: &mut CircuitBuilder<P, D>,
    state: &mut [Target; WIDTH_8],
) where
    P: RichField + Extendable<D>,
{
    for i in (0..WIDTH_8).step_by(4) {
        let t0 = builder.add(state[i], state[i + 1]);
        let t1 = builder.add(state[i + 2], state[i + 3]);
        let two_x1 = builder.add(state[i + 1], state[i + 1]);
        let two_x3 = builder.add(state[i + 3], state[i + 3]);
        let t2 = builder.add(two_x1, t1);
        let t3 = builder.add(two_x3, t0);

        let two_t1 = builder.add(t1, t1);
        let four_t1 = builder.add(two_t1, two_t1);
        let t4 = builder.add(four_t1, t3);

        let two_t0 = builder.add(t0, t0);
        let four_t0 = builder.add(two_t0, two_t0);
        let t5 = builder.add(four_t0, t2);

        let t6 = builder.add(t3, t5);
        let t7 = builder.add(t2, t4);
        state[i] = t6;
        state[i + 1] = t5;
        state[i + 2] = t7;
        state[i + 3] = t4;
    ...
    let sums = [
        builder.add(state[0], state[4]),
        builder.add(state[1], state[5]),
        builder.add(state[2], state[6]),
        builder.add(state[3], state[7]),
    ];
