...
impl MetalShared {
    fn new() -> Result<Self, String> {
        autoreleasepool(|| {
            ...
            // A long-lived production prover pays the MSL front end once per
            // process with the OS shader cache enabled, so the artifact's
            // startup saving does not apply here; compiling the source keeps
            // the kernels in lockstep with `poseidon2.metal` and avoids ever
            // silently running a stale committed artifact.
            let library = device
